...
type SavedTracksTableProps = {
  ...
  fetchTracks: (offset: number, limit: number) => Promise<{
    items: Track[]
    total: number
    hasNext: boolean
  }>
...
export function SavedTracksTable({
  ...
  const handlePageChange = async (newPage: number) => {
    setIsPending(true)
    try {
      const data = await fetchTracks(newPage * TRACKS_PER_PAGE, TRACKS_PER_PAGE)
        ...
        <tbody>
          {isPending ? (
            ...
            tracks.map((track, index) => (
              <tr
                key={track.id}
                className="transition-all duration-fast hover:bg-pink-500/5"
              >
                ...
                <td className="p-base px-md border-b border-white/[0.04] text-sm">
                  <div className="flex items-center gap-base">
                    <div
                      className="w-12 h-12 rounded-md flex-shrink-0 shadow-[0_4px_12px_rgba(236,72,153,0.2)]"
                      style={{
